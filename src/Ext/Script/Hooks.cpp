...
DEFINE_HOOK(0x723CA1, TeamMissionClass_FillIn_StringsSupport, 0xB)
{
	...
	{
		// Integer case (the most common since always).
		argument = static_cast<int>(val);
	}
	else
	{
		// New strings case
		char textArgument[0x800] = { 0 };
		...
		switch (static_cast<PhobosScripts>(action))
		{
			...
			case PhobosScripts::MoveToFriendlyStructureByID:
				...
				*/
				...
				if (sscanf(textArgument, "%s,%s", id, bwp) == 2)
				{
...
/*DEFINE_HOOK(0x69191F, check_script_values, 0x6)
{
	GET(ScriptTypeClass*, pScript, ESI);
