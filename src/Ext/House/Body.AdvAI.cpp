...
static bool IsSameSupportNetwork(const BuildingTypeClass* pTypeA,
								 const BuildingTypeClass* pTypeB)
{
	...
	const char* groupA = TechnoTypeExt::GetSelectionGroupID(
		const_cast<BuildingTypeClass*>(pTypeA));
...
static int CountOwnedBuildingInstances(HouseClass* pHouse, BuildingTypeClass* pType)
{
	if (pHouse == nullptr || pType == nullptr)
		return 0;

	int count = pHouse->ActiveBuildingTypes.GetItemCount(pType->ArrayIndex);
	const auto pExt = BuildingTypeExt::ExtMap.Find(pType);
	const bool isUpgrade = (pType->PowersUpBuilding[0] != '\0') || (pExt && !pExt->PowersUp_Buildings.empty());
	if (isUpgrade)
	{
		int upgradeCount = 0;
...
static const BuildingTypeClass* GetBasicDefense(HouseClass* pHouse,
												TechTreeTypeClass* pTechTree)
{
...
{
	int zone =
		MapClass::Instance.GetMovementZoneType(cell, MovementZone::Normal, false);
	if (zone > 0)
		return zone;

	// If the cell itself is impassable (e.g. because it contains the tree itself,
	// or is blocked by an obstacle), check adjacent cells
	static const int dx[] = {0, 1, 1, 1, 0, -1, -1, -1};
	static const int dy[] = {-1, -1, 0, 1, 1, 1, 0, -1};
	for (int i = 0; i < 8; ++i)
	{
		CellStruct neighbor(static_cast<short>(cell.X + dx[i]),
							static_cast<short>(cell.Y + dy[i]));
		if (MapClass::Instance.CoordinatesLegal(neighbor))
		{
			int nZone = MapClass::Instance.GetMovementZoneType(
				neighbor, MovementZone::Normal, false);
			if (nZone > 0)
				return nZone;
		}
	}
...
static int GetNonConflictingScanFrame(int currentFrame, int baseInterval,
									  std::initializer_list<int> otherFrames)
{
	int candidateFrame = currentFrame + baseInterval +
						 ScenarioClass::Instance->Random.RandomRanged(-10, 10);
...
static void InitializeGlobalSectors()
{
	...
	// Scan the 512x512 grid to find the real bounding box of playable cell
	// coordinates
	...
	for (int sy = yMin; sy < yMax; sy += SECTOR_SIZE)
	{
		for (int sx = xMin; sx < xMax; sx += SECTOR_SIZE)
		{
			...
			sector.BoundsMin =
				CellStruct(static_cast<short>(sx), static_cast<short>(sy));
			sector.BoundsMax = CellStruct(static_cast<short>(sx + SECTOR_SIZE),
										  static_cast<short>(sy + SECTOR_SIZE));
	...
	Debug::Log("AdvAI: Initialized & pre-scanned %d global resource sectors "
			   "(18x18). Map visible area: (%d,%d) to (%d,%d)\n",
			   static_cast<int>(GlobalResourceSectors.size()), xMin, yMin, xMax,
			   yMax);
	...
	for (size_t idx = 0; idx < GlobalResourceSectors.size(); ++idx)
	{
		...
		{
			activeSectorsCount++;
			Debug::Log("AdvAI: Startup active resource sector #%d bounds: (%d,%d) to "
					   "(%d,%d), cached resource cell: (%d,%d)\n",
					   static_cast<int>(idx), sector.BoundsMin.X, sector.BoundsMin.Y,
					   sector.BoundsMax.X - 1, sector.BoundsMax.Y - 1,
					   sector.CachedCoords.X, sector.CachedCoords.Y);
		}
	}
...
HouseExt::AdvAI_Get_Reachable_Resource_Fields(HouseClass* pHouse)
{
	...
	if (houseExt != nullptr)
	{
		...
		houseExt->NextReachableResourceScanFrame =
			Unsorted::CurrentFrame + 300; // 20-second cache (300 frames at 15 FPS)
	}
...
int HouseExt::AdvAI_GetMobileRefineryTargetCount(HouseClass* pHouse)
{
	...
	int reachableResources = static_cast<int>(
		HouseExt::AdvAI_Get_Reachable_Resource_Fields(pHouse).size());
	...
	if (pTechTree != nullptr && !pTechTree->BuildTech.empty())
	{
		...
		{
			if (pTech &&
				pHouse->ActiveBuildingTypes.GetItemCount(pTech->ArrayIndex) > 0)
			{
				hasTechCenter = true;
				break;
			}
...
bool HouseExt::AdvAI_House_Search_For_Next_Expansion_Point(HouseClass* pHouse)
{
	...
	for (const auto pBuilding : BuildingClass::Array)
	{
		if (pBuilding->IsAlive && !pBuilding->InLimbo &&
			...
			pBuilding->Type->Factory == AbstractType::BuildingType)
		{
			hasConYard = true;
			break;
		}
	...
	// attempt to pivot directly to combat crawling / beachhead towards the enemy
	// base if they are within crawling range.
	const int slaveMinerCount =
		RulesClass::Instance->PrerequisiteProcAlternate != nullptr
			...
			: 0;
	size_t refineryCount = TechTreeTypeClass::CountTotalOwnedBuildings(
		pHouse, TechTreeTypeClass::BuildType::BuildRefinery);
	refineryCount += slaveMinerCount;

	double maxDist = 80.0 +
					 (refineryCount > 3 ? (refineryCount - 3) * 10.0 : 0.0) +
					 (Unsorted::CurrentFrame / 9000.0) * 10.0;
	...
	if (needCombatTarget && refineryCount >= 3)
	{
		...
		if (pEnemy != nullptr)
		{
			...
			for (const auto pBld : pEnemy->Buildings)
			{
				if (pBld && pBld->IsAlive && !pBld->InLimbo)
				{
					for (const auto pOurBld : pHouse->Buildings)
					{
						...
						{
							double distSq = pOurBld->GetMapCoords().DistanceFromSquared(
								pBld->GetMapCoords());

							if (distSq < nearestEnemyDistSq &&
								GeneralUtils::AreZonesConnected(pOurBld->GetMapCoords(),
																pBld->GetMapCoords()))
							{
			...
			int factoryCount = TechTreeTypeClass::CountTotalOwnedBuildings(
				pHouse, TechTreeTypeClass::BuildType::BuildBarracks);
			int warFactoryCount = TechTreeTypeClass::CountTotalOwnedBuildings(
				pHouse, TechTreeTypeClass::BuildType::BuildWeapons);
			if (nearestEnemyDistSq <= maxDistSq && factoryCount > 0 &&
				warFactoryCount > 0)
			{
				const BuildingClass* pOurConYard =
					pHouse->ConYards.Count > 0 ? pHouse->ConYards[0] : nullptr;
				const CellStruct baseCenter = pOurConYard != nullptr
				...
				// Fallback to base center if no individual building is found/valid (and
				// center not blacklisted)
				...
				Debug::Log("AdvAI Combat Crawl Evaluation: House %d: refineryCount=%d, "
						   "enemyIndex=%d, nearestEnemyDist=%.1f (Limit=%.1f), "
						   "hasFactories=%s, hasTarget=%s\n",
						   pHouse->ArrayIndex, refineryCount, pEnemy->ArrayIndex,
						   (nearestEnemyDistSq != std::numeric_limits<double>::max()
								? sqrt(nearestEnemyDistSq)
								: -1.0),
						   maxDist,
				...
				if (bestEnemyTarget.X > 0 && bestEnemyTarget.Y > 0)
				{
					...
					Debug::Log("AdvAI SearchExpansion: House %d starting COMBAT crawling "
							   "towards enemy %d (%s) at (%d,%d).\n",
							   pHouse->ArrayIndex, pEnemy->ArrayIndex, pEnemy->Type->ID,
							   bestEnemyTarget.X, bestEnemyTarget.Y);
					foundAny = true;
	...
	if (target.X == 0 || target.Y == 0)
	{
		...
		for (const auto pTerrain : TerrainClass::Array)
		{
			if (pTerrain && pTerrain->IsAlive && pTerrain->Type->SpawnsTiberium)
			{
				...
				for (const auto pBuilding : pHouse->Buildings)
				{
					...
					{
						const double dist =
							treeCoords.DistanceFrom(pBuilding->GetMapCoords());
				...
				if (!occupied)
				{
					Debug::Log("AdvAI ExpansionSearch Safeguard: House %d: Tree %s at "
							   "(%d,%d) -> Available (Nearest friendly refinery: %s at "
							   "(%d,%d), dist %.1f cells)\n",
							   pHouse->ArrayIndex, pTerrain->Type->ID, treeCoords.X,
							   treeCoords.Y,
					...
					if (pNearestStructure == nullptr)
						Debug::Log("AdvAI ExpansionSearch Safeguard: House %d: Tree %s at "
								   "(%d,%d) -> Unreachable (no buildings found)\n",
								   pHouse->ArrayIndex, pTerrain->Type->ID, treeCoords.X,
								   treeCoords.Y);
					else
						Debug::Log(
							"AdvAI ExpansionSearch Safeguard: House %d: Tree %s at (%d,%d) "
							"-> Reachable. Nearest structure: %s at dist %.1f cells\n",
							pHouse->ArrayIndex, pTerrain->Type->ID, treeCoords.X,
							treeCoords.Y, pNearestStructure->Type->ID, minStructureDist);
		...
		if (target.X > 0 && target.Y > 0)
			Debug::Log("AdvAI ExpansionSearch Safeguard: House %d: Recovered target "
					   "(%d,%d) because it has no refinery within its radius.\n",
					   pHouse->ArrayIndex, target.X, target.Y);
	}

	if (target.X == 0 || target.Y == 0)
	{
		// If we couldn't find any Tiberium fields, crawl towards the enemy if they
		// are within reach
		const HouseClass* pEnemy = nullptr;
		if (pHouse->EnemyHouseIndex >= 0 &&
			pHouse->EnemyHouseIndex < HouseClass::Array.Count)
		...
		if (pEnemy != nullptr)
		{
			...
			Debug::Log("AdvAI: House %d: All Tiberium fields taken. Checking combat "
					   "crawling. nearestEnemyDist=%.1f (Limit=%.1f), hasTarget=%s\n",
					   pHouse->ArrayIndex,
					   (nearestEnemyDistSq != std::numeric_limits<double>::max()
							? sqrt(nearestEnemyDistSq)
							: -1.0),
					   maxDist, (enemyTarget.X > 0 ? "YES" : "NO"));

			// If the enemy base is within crawling range
			if (nearestEnemyDistSq <= maxDistSq && enemyTarget.X > 0 &&
				enemyTarget.Y > 0)
			{
				ext->CombatCrawlingTarget = enemyTarget;
				Debug::Log("AdvAI ExpansionSearch: House %d: All Tiberium fields "
						   "taken. Crawling towards enemy House %d (%s) base at "
						   "(%d,%d) at dist %.1f cells (Limit: %.1f).\n",
						   pHouse->ArrayIndex, pEnemy->ArrayIndex, pEnemy->Type->ID,
						   enemyTarget.X, enemyTarget.Y, std::sqrt(nearestEnemyDistSq),
						   maxDist);
				foundAny = true;
			}
		}
...
bool HouseExt::AdvAI_Can_Build_Building(HouseClass* pHouse,
										BuildingTypeClass* pBuildingType,
										bool checkPrereqs, bool isTechTree)
{
	...
	if (pBuildingType->ResourceGatherer && !inRecurse)
	{
		...
		int maxAllowed =
			isMobileOnly ? AdvAI_GetMobileRefineryTargetCount(pHouse) : 5;
	...
	// This prevents the AI queue from getting locked by repeatedly building
	// structures it cannot place.
	const auto houseExt = ExtMap.Find(pHouse);
	...
	if (!pHouse->InOwners(pBuildingType))
		return false;

	// Check TechLevel
	if (pBuildingType->TechLevel != -1)
	{
	...
	bool hasBuildingFactory = false;
	for (const auto pBld : pHouse->Buildings)
	{
		if (pBld && pBld->IsAlive && !pBld->InLimbo && pBld->Type &&
			(pBld->Type->ConstructionYard ||
			 pBld->Type->Factory == AbstractType::BuildingType))
		{
			hasBuildingFactory = true;
			break;
		}
	}

	if (!hasBuildingFactory)
		return false;
	...
	if (isUpgrade)
	{
		...
		for (const auto pBld : pHouse->Buildings)
		{
			if (pBld && pBld->IsAlive && !pBld->InLimbo)
			{
				bool isEligibleParent = false;
				if (pBuildingType->PowersUpBuilding[0] != '\0' && _stricmp(pBld->Type->ID, pBuildingType->PowersUpBuilding) == 0)
					isEligibleParent = true;
				if (!isEligibleParent && pExt != nullptr)
				{
					for (const auto pTargetType : pExt->PowersUp_Buildings)
					{
						if (pTargetType == pBld->Type)
						{
							isEligibleParent = true;
							break;
						}
...
prereqsChecked:
	...
	if (!pExt->PowersUp_Buildings.empty())
	{
		...
		{
			if (pPowerUpBld != nullptr &&
				(TechTreeTypeClass::TotalBuildPower.count(pPowerUpBld) > 0 ||
				 TechTreeTypeClass::TotalBuildAdvancedPower.count(pPowerUpBld) > 0))
			{
				isPowerUpgrade = true;
				break;
			}
...
BuildingTypeClass* HouseExt::AdvAI_Find_Next_Buildable_Prerequisite(
	HouseClass* pHouse,
	BuildingTypeClass* pTargetType,
	BuildingTypeClass* pRootGoalType,
	std::set<BuildingTypeClass*>& visited,
	bool& outIsSubPrereq)
{
	if (pHouse == nullptr || pTargetType == nullptr)
		return nullptr;

	if (visited.count(pTargetType) > 0)
		return nullptr; // Cycle / recursion loop protection

	visited.insert(pTargetType);

	const auto pExt = TechnoTypeExt::Fetch(pTargetType);
	if (pExt == nullptr)
		return nullptr;

	// Helper lambda to collect all candidate buildings from a prerequisite integer token
	auto getBuildingsForPrereqToken = [&](int token) -> std::vector<BuildingTypeClass*>
	{
		std::vector<BuildingTypeClass*> candidates;
		if (token >= 0 && token < BuildingTypeClass::Array.Count)
		{
			BuildingTypeClass* pBld = BuildingTypeClass::Array[token];
			if (pBld != nullptr)
				candidates.push_back(pBld);
		}
		else if (token < 0)
		{
			// Generic prerequisite token
			int absIdx = std::abs(token);
			if (absIdx < RulesExt::Global()->GenericPrerequisites.Count)
			{
				const auto& genList = RulesExt::Global()->GenericPrerequisites.GetItem(absIdx);
				for (int bldIdx : genList)
				{
					if (bldIdx >= 0 && bldIdx < BuildingTypeClass::Array.Count)
					{
						BuildingTypeClass* pBld = BuildingTypeClass::Array[bldIdx];
						if (pBld != nullptr)
							candidates.push_back(pBld);
					}
				}
	...
	struct ListEvalResult
	{
		bool isSatisfied;
		std::vector<BuildingTypeClass*> directlyBuildable;
		std::vector<BuildingTypeClass*> subBuildable;
	};

	auto evaluatePrereqList = [&](const auto& tokenList) -> ListEvalResult
	{
		ListEvalResult res;
		...
		for (int token : tokenList)
		{
			...
			std::vector<BuildingTypeClass*> directlyBuildableForToken;
			std::vector<BuildingTypeClass*> subBuildableForToken;
			...
			for (auto pBld : possibleBuildings)
			{
				...
				{
					bool isSub = false;
					BuildingTypeClass* pSub = AdvAI_Find_Next_Buildable_Prerequisite(
						pHouse, pBld, pRootGoalType, visited, isSub);
					if (pSub != nullptr)
						subBuildableForToken.push_back(pSub);
				}
			}

			if (!directlyBuildableForToken.empty())
			{
				int r = ScenarioClass::Instance->Random.RandomRanged(
					0, static_cast<int>(directlyBuildableForToken.size()) - 1);
				res.directlyBuildable.push_back(directlyBuildableForToken[r]);
			}
			else if (!subBuildableForToken.empty())
			{
				int r = ScenarioClass::Instance->Random.RandomRanged(
					0, static_cast<int>(subBuildableForToken.size()) - 1);
				res.subBuildable.push_back(subBuildableForToken[r]);
			}
		}
	...
	std::vector<BuildingTypeClass*> allDirectlyBuildable;
	std::vector<BuildingTypeClass*> allSubBuildable;

	if (!pExt->Prerequisite_ListVector.empty())
	{
		for (const auto& altList : pExt->Prerequisite_ListVector)
		{
			auto listRes = evaluatePrereqList(altList);
			if (!listRes.isSatisfied)
			{
				allDirectlyBuildable.insert(allDirectlyBuildable.end(),
											listRes.directlyBuildable.begin(),
											listRes.directlyBuildable.end());
				allSubBuildable.insert(allSubBuildable.end(),
									   listRes.subBuildable.begin(),
									   listRes.subBuildable.end());
			}
		}
	}
	else if (!pExt->Prerequisite.empty())
	{
		auto listRes = evaluatePrereqList(pExt->Prerequisite);
		if (!listRes.isSatisfied)
		{
			allDirectlyBuildable.insert(allDirectlyBuildable.end(),
										listRes.directlyBuildable.begin(),
	...
	if (!allDirectlyBuildable.empty())
	{
		...
		int r = ScenarioClass::Instance->Random.RandomRanged(
			0, static_cast<int>(allDirectlyBuildable.size()) - 1);
	...
	if (!allSubBuildable.empty())
	{
		outIsSubPrereq = true;
		int r = ScenarioClass::Instance->Random.RandomRanged(
			0, static_cast<int>(allSubBuildable.size()) - 1);
		return allSubBuildable[r];
	}
 ...
 *  Author: Rampastring
 */
bool HouseExt::AdvAI_Is_Under_Start_Rush_Threat(HouseClass* pHouse,
												int enemyAircraftValue)
{
	// If the game has progressed for long enough, it is no longer considered a
	// start rush.
	...
	for (const auto pInfantry : InfantryClass::Array)
	{
		...
		// for human infantry.
		houseInfantryStrength[pInfantry->Owner->ArrayIndex] +=
			pInfantry->Type->Points * 2;
	}
...
/**
 *  Gets the building that the Advanced AI should build in its current game
 * situation.
 *
 *  Author: Rampastring
 */
											  ...
											  CellStruct threatCenter)
{
	...
	{
		if (pObj && pObj->IsAlive && !pObj->InLimbo && pObj->Owner != pHouse &&
			!pHouse->IsAlliedWith(pObj->Owner))
		{
	...
	// Treat enemy structures (defenses, conyards) as armored ground threats
	// (Vehicles)
	for (const auto pBld : BuildingClass::Array)
	{
...
HouseExt::AdvAI_Evaluate_Get_Best_Building(HouseClass* pHouse)
{
	constexpr bool LogVerboseAdvAI = false;

	auto canBuildFunction = [pHouse](auto&& PH1)
	{
		return AdvAI_Can_Build_Building(pHouse, std::forward<decltype(PH1)>(PH1),
										true, true);
	};

	const auto houseExt = ExtMap.Find(pHouse);

	auto GetTargetBuildCount = [&](BuildingTypeClass* pType, int defaultCount,
								   TechTreeTypeClass* pTechTree) -> int
	{
		// If BuildOtherCounts in the TechTree explicitly specifies a count for this
		// building, that takes priority and overrides AIBuildCounts/AIExtraCounts.
		...
		const unsigned int difficulty =
			pHouse->GetAIDifficultyIndex(); // Hard=0, Normal=1, Easy=2
		...
		if (pTypeExt->AIBuildCounts.size() > difficulty)
		{
			...
			if (pTypeExt->AIExtraCounts.size() > difficulty)
			{
				...
				if (extraMax > 0)
					extraCount =
						ScenarioClass::Instance->Random.RandomRanged(0, extraMax);
			}
	...
	{
		pPrimaryTechTree =
			TechTreeTypeClass::GetForSide(houseExt->OwnerObject()->Type->SideIndex);
		pSecondaryTechTree =
			TechTreeTypeClass::GetForSide(houseExt->OwnerObject()->Type->SideIndex);
	...
	// If we can't build using our primary tech tree, choose another one. If there
	// is none, we shouldn't be here (no ConYard).
	...
	// If we can't build using our secondary tech tree, reset it to the primary
	// one
	...
	if (pSecondaryTechTree->IsCompleted(pHouse, canBuildFunction))
	{
		...
		{
			if (pTechTree->IsSuitable(pHouse) &&
				!pTechTree->IsCompleted(pHouse, canBuildFunction))
			{
	...
	///	Handles the main expansion
	{
		...
		const BuildingTypeClass* pPowerPlantToBuild =
			AdvAI_BuildAtLeastNOfSideAndMInTotal(
				pHouse, pPrimaryTechTree, TechTreeTypeClass::BuildType::BuildPower,
				1, 1);
		if (pPowerPlantToBuild != nullptr)
		{
			Debug::Log(
				"AdvAI: Making AI build %s because it has 0 basic power plants\n",
		...
		if (pHouse->AIDifficulty < AIDifficulty::Hard &&
			pHouse->Balance >=
				RulesClass::Instance->AIAlternateProductionCreditCutoff)
		{
			const BuildingTypeClass* pBarracksToBuild =
				AdvAI_BuildAtLeastNOfSideAndMInTotal(
					pHouse, pPrimaryTechTree,
					TechTreeTypeClass::BuildType::BuildBarracks, 1, 1);
			if (pBarracksToBuild != nullptr)
			{
				Debug::Log("AdvAI: Making AI build %s because it does not have a "
						   "Barracks at all.\n",
		...
		// Check whether we're in threat of being rushed right in the beginning of
		// the game.
		...
		// Build refinery if we're expanding and we're not under immediate air rush
		// threat
		if (!isUnderThreat)
		{
			const BuildingTypeClass* pOurRefinery =
				pPrimaryTechTree->GetRandomBuildable(
		...
		const int slaveMinerCount =
			RulesClass::Instance->PrerequisiteProcAlternate != nullptr
				? pHouse->ActiveUnitTypes.GetItemCount(
					  RulesClass::Instance->PrerequisiteProcAlternate->ArrayIndex)
				: 0;
		size_t refineryCount =
			houseExt->PrimaryTechTreeType->CountSideOwnedBuildings(
				pHouse, TechTreeTypeClass::BuildType::BuildRefinery);
		refineryCount += slaveMinerCount;

		// Build a refinery if we have 0 left. Can't use the generic function as we
		// need to also count slave miners
		const BuildingTypeClass* pRefineryToBuild =
			AdvAI_BuildAtLeastNOfSideAndMInTotal(
				pHouse, pPrimaryTechTree,
				TechTreeTypeClass::BuildType::BuildRefinery, 0, 1, slaveMinerCount);
		...
		if (!pHouse->PowerBlackoutTimer.HasTimeLeft())
		{
			for (const auto pBuilding : BuildingClass::Array)
			{
				if (pBuilding->Owner == pHouse && pBuilding->IsAlive &&
					!pBuilding->InLimbo && !pBuilding->IsUnderEMP())
				{
					if (!pBuilding->HasPower ||
						(pBuilding->Type->TogglePower && !pBuilding->StuffEnabled))
					{
		...
		const int realDrain = GetRealPowerDrain(pHouse);
		const int reqSurplus = pHouse->PowerSurplus > 0
								   ? pHouse->PowerSurplus
								   : RulesClass::Instance->PowerSurplus;
		const bool isLowOnPower =
			(pHouse->PowerOutput - realDrain < reqSurplus) || hasUnpoweredBuildings;

		if (isLowOnPower)
		{
			const BuildingTypeClass* pOurAdvancedPowerPlant =
				pPrimaryTechTree->GetRandomBuildable(
					TechTreeTypeClass::BuildType::BuildAdvancedPower,
					canBuildFunction);
			if (pOurAdvancedPowerPlant != nullptr)
			{
				Debug::Log("AdvAI: House %d making AI build %s because real power surplus is low "
						   "(Output: %d, Real Drain: %d, Req Surplus: %d, HasUnpowered: %s)\n",
						   pHouse->ArrayIndex, pOurAdvancedPowerPlant->Name,
						   pHouse->PowerOutput, realDrain, reqSurplus,
						   hasUnpoweredBuildings ? "YES" : "NO");
				return pOurAdvancedPowerPlant;
			}

			const BuildingTypeClass* pOurPowerPlant =
				pPrimaryTechTree->GetRandomBuildable(
			...
			if (pOurPowerPlant != nullptr)
			{
				Debug::Log("AdvAI: House %d making AI build %s because real power surplus is low "
						   "(Output: %d, Real Drain: %d, Req Surplus: %d, HasUnpowered: %s)\n",
						   pHouse->ArrayIndex, pOurPowerPlant->Name,
						   pHouse->PowerOutput, realDrain, reqSurplus,
						   hasUnpoweredBuildings ? "YES" : "NO");
		...
		// Competitive scaling: if a competitor has at least double our barracks
		...
		AdvAI_Recycle_Furthest_Factory(pHouse, AbstractType::InfantryType, false,
									   optimalBarracksCount,
									   houseExt->NextExpansionPointLocation);

		const BuildingTypeClass* pBarracksToBuild =
			AdvAI_BuildAtLeastNOfSideAndMInTotal(
				pHouse, pPrimaryTechTree,
				TechTreeTypeClass::BuildType::BuildBarracks, 1,
				optimalBarracksCount);

		if (pBarracksToBuild != nullptr)
		{
			Debug::Log("AdvAI: Making AI build %s because it does not have enough "
					   "Barracks. Wanted: %d (Competitor max: %d)\n",
					   pBarracksToBuild->Name, optimalBarracksCount,
					   maxBarracksOwnedByOther);

			return pBarracksToBuild;
		}

		// Get defenses and calculate for deficiencies on them before making further
		// decisions.
		...
		if (LogVerboseAdvAI)
			Debug::Log("AdvAI Eval: House %d (%s), TechTree: %s, Raw BuildDefense "
					   "count in INI: %u\n",
					   pHouse->ArrayIndex, pHouse->Type->ID,
					   pPrimaryTechTree->Name.data(),
					   pPrimaryTechTree->BuildDefense.size());
		for (const auto pDefType : pPrimaryTechTree->BuildDefense)
		{
			bool canBuild = AdvAI_Can_Build_Building(pHouse, pDefType, true, true);
			if (LogVerboseAdvAI)
				Debug::Log("AdvAI Eval: House %d checking Defense INI item %s -> "
						   "CanBuild: %s (AIBuildThis=%d, TechLvl=%d vs HouseLvl=%d, "
						   "CanExpectToBuild=%d)\n",
						   pHouse->ArrayIndex, pDefType ? pDefType->ID : "NULL",
						   canBuild ? "YES" : "NO",
						   pDefType ? pDefType->AIBuildThis : 0,
						   pDefType ? pDefType->TechLevel : -1, pHouse->TechLevel,
						   pDefType ? pHouse->CanExpectToBuild(pDefType) : 0);
		}

		auto buildableDefenses = pPrimaryTechTree->GetBuildable(
			TechTreeTypeClass::BuildType::BuildDefense, canBuildFunction);
		if (LogVerboseAdvAI)
			Debug::Log("AdvAI Eval: House %d (%s), TechTree: %s, BuildableDefenses "
					   "count: %u\n",
					   pHouse->ArrayIndex, pHouse->Type->ID,
					   pPrimaryTechTree->Name.data(), buildableDefenses.size());
		...
		for (const auto pDefense : buildableDefenses)
		{
			...
			{
				// Applying a 99% penalty to the score if it would overload our power
				// grid.
			...
			{
				// We can't even afford it right now, penalize it heavily so we choose
				// something buildable.
				antiInfantryScore *= 0.01;
				antiVehicleScore *= 0.01;
				antiAirScore *= 0.01;
			...
			else if (currentMoney < 2000)
			{
				...
				// We normalize by a baseline cost (e.g. 500) to keep the magnitude
				// comparable.
			...
			// Apply an 85% score reduction for defenses without built-in weapons
			// (e.g. empty Battle Bunkers)
			...
			if (LogVerboseAdvAI)
				Debug::Log("AdvAI Eval: House %d Defense Option %s -> Cost: %d, "
						   "PowerDrain: %d, InfVal: %d (Score: %.4f), ArmVal: %d "
						   "(Score: %.4f), AirVal: %d (Score: %.4f)\n",
						   pHouse->ArrayIndex, pDefense->ID, pDefense->Cost,
						   pDefense->PowerDrain, pDefense->AntiInfantryValue,
						   antiInfantryScore, pDefense->AntiArmorValue,
						   antiVehicleScore, pDefense->AntiAirValue, antiAirScore);

			if (pDefense->AntiInfantryValue > 0 && antiInfantryScore > 0.0)
			{
				antiInfantryCandidates.push_back(pDefense);
				antiInfantryWeights.push_back(
					static_cast<int>(antiInfantryScore * 100.0));
			}
			...
			if (pDefense->AntiArmorValue > 0 && antiVehicleScore > 0.0)
			{
				...
				antiVehicleWeights.push_back(
					static_cast<int>(antiVehicleScore * 100.0));
			}
		...
		if (!antiInfantryWeights.empty())
		{
			...
			int chosenIndex =
				GeneralUtils::ChooseOneWeighted(dice, &antiInfantryWeights);
			if (chosenIndex >= 0)
				ourAntiInfantryDefense = const_cast<BuildingTypeClass*>(
					antiInfantryCandidates[chosenIndex]);
		}
		...
		if (!antiVehicleWeights.empty())
		{
			const double dice = ScenarioClass::Instance->Random.RandomDouble();
			int chosenIndex =
				GeneralUtils::ChooseOneWeighted(dice, &antiVehicleWeights);
			if (chosenIndex >= 0)
				ourAntiVehicleDefense =
					const_cast<BuildingTypeClass*>(antiVehicleCandidates[chosenIndex]);
		}
		...
		if (!antiAirWeights.empty())
		{
			...
			if (chosenIndex >= 0)
				ourAntiAirDefense =
					const_cast<BuildingTypeClass*>(antiAirCandidates[chosenIndex]);
		}

		if (ourAntiInfantryDefense != nullptr)
			bestAntiInfantryScore *= 1.15;
		if (ourAntiVehicleDefense != nullptr)
			bestAntiVehicleScore *= 1.15;
		if (ourAntiAirDefense != nullptr)
			bestAntiAirScore *= 1.15;
		...
		if (houseExt->LastAttackerType != nullptr && wasRecentlyAttacked)
		{
			...
			else if (pAttackerType->WhatAmI() == AbstractType::InfantryType)
			{
			...
			{
				auto const pUnitType = specific_cast<UnitTypeClass*>(pAttackerType);
		...
		if (LogVerboseAdvAI)
			Debug::Log(
				"AdvAI Eval: House %d Totals -> BaseOptVal: %d, InfVal: %d (Defic: %d), "
				"ArmVal: %d (Defic: %d), AirVal: %d (Defic: %d). BestCandidates -> "
				"Inf: %s (Own: %d, Limit: %d), Arm: %s (Own: %d, Limit: %d), Air: %s "
				"(Own: %d, Limit: %d)\n",
				pHouse->ArrayIndex, baseOptimalDefenseValue, antiInfantryDefenseValue,
				antiInfDeficiency, antiVehicleDefenseValue, antiVehicleDeficiency,
				antiAirDefenseValue, antiAirDeficiency,
				ourAntiInfantryDefense ? ourAntiInfantryDefense->ID : "None",
				ourAntiInfantryDefense ? pHouse->ActiveBuildingTypes.GetItemCount(
											 ourAntiInfantryDefense->ArrayIndex)
									   : 0,
				ourAntiInfantryDefense ? ourAntiInfantryDefense->BuildLimit : 0,
				ourAntiVehicleDefense ? ourAntiVehicleDefense->ID : "None",
				ourAntiVehicleDefense ? pHouse->ActiveBuildingTypes.GetItemCount(
											ourAntiVehicleDefense->ArrayIndex)
									  : 0,
				ourAntiVehicleDefense ? ourAntiVehicleDefense->BuildLimit : 0,
				ourAntiAirDefense ? ourAntiAirDefense->ID : "None",
				ourAntiAirDefense ? pHouse->ActiveBuildingTypes.GetItemCount(
										ourAntiAirDefense->ArrayIndex)
								  : 0,
				ourAntiAirDefense ? ourAntiAirDefense->BuildLimit : 0);

		// Detect if any active enemy has built a superweapon (Arms Race Trigger)
		...
		if (isParanoid || hasSomethingToProtect)
		{
			...
			if (ScenarioClass::Instance->Random.RandomRanged(0, 99) < rollChance)
			{
				...
				else
				{
					...
					if (antiAirDeficiency > maxDeficiency &&
						ourAntiAirDefense != nullptr)
					{
				...
				{
					Debug::Log(
						"AdvAI: Making AI build %s (highest deficiency, "
						"paranoid/protecting). InfDef: %d, VehDef: %d, AirDef: %d\n",
		...
		if (!isUnderThreat || (antiInfDeficiency <= 0 && antiAirDeficiency <= 0))
		{
			...
			const bool hasSuperWeaponGoal =
				GameModeOptionsClass::Instance.SWAllowed &&
				(!pPrimaryTechTree->BuildSuperWeapon.empty() ||
				 !pPrimaryTechTree->BuildSuperWeaponRandom.empty());

			bool hasCompletedSuperWeapons = true;
			if (hasSuperWeaponGoal)
			{
				for (const auto pSW : pPrimaryTechTree->BuildSuperWeapon)
				{
					if (pSW != nullptr && !pSW->Unbuildable)
					{
						const int target = GetTargetBuildCount(pSW, 1, pPrimaryTechTree);
						if (CountOwnedBuildingInstances(pHouse, pSW) < target)
						{
							hasCompletedSuperWeapons = false;
							break;
						}
					}
				}

				if (hasCompletedSuperWeapons)
				{
					for (const auto pSW : pPrimaryTechTree->BuildSuperWeaponRandom)
					{
						if (pSW != nullptr && !pSW->Unbuildable)
						{
							const int target = GetTargetBuildCount(pSW, 1, pPrimaryTechTree);
			...
			auto evaluateWeaponsFactory = [&]() -> const BuildingTypeClass*
			{
				...
				if (!hasTechCenter || !hasCompletedSuperWeapons)
					optimalWeaponsCount = 1;

				// Competitive scaling: if a competitor has at least double our War
				// Factory count, increase our limit
				if (hasCompletedSuperWeapons && maxWFOwnedByOther >= ourWFCount * 2 && ourWFCount > 0)
					optimalWeaponsCount = std::max(optimalWeaponsCount,
												   static_cast<size_t>(ourWFCount + 1));

				// Enforce difficulty-based safety cap for War Factories (Easy: 6,
				// Normal: 8, Hard: 12) Reduced in naval mode (Easy: 2, Normal: 3, Hard:
				// 4) to save space on islands
				size_t maxWFLimit = 8;
				if (isNavalMode)
				{
					maxWFLimit = 3;
				...
				AdvAI_Recycle_Furthest_Factory(pHouse, AbstractType::UnitType, false,
											   optimalWeaponsCount,
											   houseExt->NextExpansionPointLocation);

				const BuildingTypeClass* pWeaponsFactoryToBuild =
					AdvAI_BuildAtLeastNOfSideAndMInTotal(
						pHouse, pPrimaryTechTree,
						TechTreeTypeClass::BuildType::BuildWeapons, 1,
						optimalWeaponsCount);

				if (pWeaponsFactoryToBuild != nullptr)
				{
					Debug::Log(
						"AdvAI: Making AI build %s because it does not have enough "
						"Weapons Factories. Wanted: %d (Competitor max: %d)\n",
			...
			auto evaluateNavalYards = [&]() -> const BuildingTypeClass*
			{
				...
				if (hasCompletedSuperWeapons && static_cast<size_t>(enemyNavalYardsCount) > optimalNavalYardCount)
					optimalNavalYardCount = static_cast<size_t>(enemyNavalYardsCount);

				// Apply difficulty-based safety cap for Naval Yards (Easy: 3, Normal:
				// 5, Hard: 8) Under naval mode, the cap increases to (Easy: 4, Normal:
				// 6, Hard: 9) And can dynamically scale up to the enemy's shipyard
				// count - 1
				...
				AdvAI_Recycle_Furthest_Factory(pHouse, AbstractType::UnitType, true,
											   optimalNavalYardCount,
											   houseExt->NextExpansionPointLocation);

				const BuildingTypeClass* pNavalYardToBuild =
					AdvAI_BuildAtLeastNOfSideAndMInTotal(
						pHouse, pPrimaryTechTree,
						TechTreeTypeClass::BuildType::BuildNavalYard, 1,
						optimalNavalYardCount);
				if (pNavalYardToBuild != nullptr)
				{
					Debug::Log("AdvAI: Making AI build %s because it does not have "
							   "enough Naval Yards. Wanted: %d (Enemy max: %d)\n",
							   pNavalYardToBuild->Name, optimalNavalYardCount,
							   enemyNavalYardsCount);
			...
			int minRefineryCount =
				RulesExt::Global()->AdvancedAI_MinimumRefineryCount;
			bool isMobileOnly = AdvAI_IsMobileRefineryHouse(pHouse);

			if (isMobileOnly)
				minRefineryCount = AdvAI_GetMobileRefineryTargetCount(pHouse);
			else if (!hasTechCenter || !hasCompletedSuperWeapons)
			...
			{
				pRefineryToBuild = AdvAI_BuildAtLeastNOfSideAndMInTotal(
					pHouse, pPrimaryTechTree,
					TechTreeTypeClass::BuildType::BuildRefinery, 1, minRefineryCount,
					slaveMinerCount);
				if (pRefineryToBuild != nullptr)
				{
					houseExt->NextExpansionPointLocation = CellStruct(0, 0);
					houseExt->ShouldBuildRefinery = false;
					Debug::Log("AdvAI: Making AI build %s because it only has too few "
		...
		// If superweapons are allowed in the game mode, evaluate the sequential list of BuildSuperWeapon
		if (GameModeOptionsClass::Instance.SWAllowed &&
			!pPrimaryTechTree->BuildSuperWeapon.empty())
		{
			for (const auto pSWType : pPrimaryTechTree->BuildSuperWeapon)
			{
				if (pSWType == nullptr || pSWType->Unbuildable)
					continue;
				...
				const int ownedCount =
					CountOwnedBuildingInstances(pHouse, pSWType);

				if (ownedCount >= targetCount)
					continue; // Already built to target count, move to next in list

				// Check if the superweapon can be built directly right now
				if (AdvAI_Can_Build_Building(pHouse, pSWType, true))
				{
					Debug::Log("AdvAI: House %d targeting SuperWeapon %s -> All "
							   "prerequisites met. Making AI build %s!\n",
							   pHouse->ArrayIndex, pSWType->Name, pSWType->Name);
					return pSWType;
				}
				else
				{
					// Prerequisites missing: recursively resolve next buildable
					// prerequisite in dependency tree
					std::set<BuildingTypeClass*> visited;
					bool isSubPrereq = false;
					BuildingTypeClass* pPrereqToBuild =
						AdvAI_Find_Next_Buildable_Prerequisite(
							pHouse, pSWType, pSWType, visited, isSubPrereq);

					if (pPrereqToBuild != nullptr)
					{
						if (isSubPrereq)
							Debug::Log("AdvAI: House %d targeting SuperWeapon %s -> Needs "
									   "sub-prerequisite %s. Making AI build %s.\n",
									   pHouse->ArrayIndex, pSWType->Name,
									   pPrereqToBuild->Name, pPrereqToBuild->Name);
						else
							Debug::Log("AdvAI: House %d targeting SuperWeapon %s -> Needs "
									   "prerequisite %s. Making AI build %s.\n",
									   pHouse->ArrayIndex, pSWType->Name,
									   pPrereqToBuild->Name, pPrereqToBuild->Name);
		...
		// BuildSuperWeaponRandom evaluation
		// If superweapons are allowed, evaluate the pool of random superweapons
		if (GameModeOptionsClass::Instance.SWAllowed &&
			!pPrimaryTechTree->BuildSuperWeaponRandom.empty())
		{
			std::vector<BuildingTypeClass*> swRandomCandidates;
			for (const auto pSWType : pPrimaryTechTree->BuildSuperWeaponRandom)
			{
				if (pSWType == nullptr || pSWType->Unbuildable)
					continue;
			...
			{
				BuildingTypeClass* pChosenSW =
					swRandomCandidates[ScenarioClass::Instance->Random.RandomRanged(
						0, static_cast<int>(swRandomCandidates.size()) - 1)];

				Debug::Log("AdvAI: Making AI build %s from BuildSuperWeaponRandom.\n",
						   pChosenSW->Name);
		...
		const BuildingTypeClass* pHelipadType =
			pPrimaryTechTree->GetRandomBuildable(
				TechTreeTypeClass::BuildType::BuildHelipad, canBuildFunction);
		if (pHelipadType != nullptr)
		{
			const int docksPerHelipad =
				pHelipadType->NumberOfDocks > 0 ? pHelipadType->NumberOfDocks : 1;
			...
			for (const auto pBld : pHouse->Buildings)
			{
				if (pBld && pBld->IsAlive && !pBld->InLimbo)
				{
					...
					{
						const int docks =
							pBld->Type->NumberOfDocks > 0 ? pBld->Type->NumberOfDocks : 1;
			...
			// (including those flying or attacking)
			int totalAircraft = 0;
			for (const auto pAircraft : AircraftClass::Array)
			{
				if (pAircraft && pAircraft->IsAlive && !pAircraft->InLimbo &&
					pAircraft->Owner == pHouse)
				{
			...
			// Also count actual occupied docks just in case some custom planes are
			// docked but not marked AirportBound
			...
			if (pEnemyHelipad != nullptr)
			{
				...
				{
					if (pBuilding->IsAlive && !pBuilding->InLimbo &&
						pBuilding->Owner == pEnemyHelipad)
					{
						if (pBuilding->Type->Helipad)
							enemyHelipadDocks += pBuilding->Type->NumberOfDocks > 0
													 ? pBuilding->Type->NumberOfDocks
													 : 1;
					}
				}
			}
			...
			if (isNavalMode && pHouse->AIDifficulty != AIDifficulty::Easy)
			{
				const size_t targetHelipadsToMatchEnemyDocks =
					(static_cast<size_t>(enemyHelipadDocks) + docksPerHelipad - 1) /
					docksPerHelipad;
		...
		if (!pPrimaryTechTree->PreBuildOtherRandom.empty())
		{
			std::vector<BuildingTypeClass*> preRandomCandidates;
			for (const auto pBldType : pPrimaryTechTree->PreBuildOtherRandom)
			{
				if (!pBldType || pBldType->Unbuildable)
					continue;

				const bool canBuild = canBuildFunction(pBldType);
				const int targetCount =
					GetTargetBuildCount(pBldType, 1, pPrimaryTechTree);
				const int ownedCount =
					CountOwnedBuildingInstances(pHouse, pBldType);

				if (canBuild && ownedCount < targetCount)
					preRandomCandidates.push_back(pBldType);
			}

			if (!preRandomCandidates.empty())
			{
				BuildingTypeClass* pChosen =
					preRandomCandidates[ScenarioClass::Instance->Random.RandomRanged(
						0, static_cast<int>(preRandomCandidates.size()) - 1)];
		...
		for (const auto buildOtherPair : pPrimaryTechTree->BuildOtherCountMap)
		{
			...
			const int targetCount = GetTargetBuildCount(
				buildOtherPair.first, buildOtherPair.second, pPrimaryTechTree);
			if (CountOwnedBuildingInstances(
					pHouse, buildOtherPair.first) < targetCount)
			{
				Debug::Log("AdvAI: Making AI build %s because it does not have enough "
						   "of it. Wanted: %d\n",
		...
		// PostBuildOtherRandom evaluation (evaluated BEFORE generic AIBuildThis array loop)
		if (!pPrimaryTechTree->PostBuildOtherRandom.empty())
		{
			std::vector<BuildingTypeClass*> postRandomCandidates;
			for (const auto pBldType : pPrimaryTechTree->PostBuildOtherRandom)
			{
				if (!pBldType || pBldType->Unbuildable)
					continue;
			...
			{
				BuildingTypeClass* pChosen =
					postRandomCandidates[ScenarioClass::Instance->Random.RandomRanged(
						0, static_cast<int>(postRandomCandidates.size()) - 1)];
		...
		// BuildAlliedFallback evaluation
		// If BuildOffAlly is enabled and BuildAlliedFallback has structures defined,
		// establish or maintain a fallback outpost in one designated ally's base.
		...
		if (buildOffAlly && !pPrimaryTechTree->BuildAlliedFallback.empty() && pHouse->Balance > 2000)
		{
			struct AllyCandidate
			{
				HouseClass* pHouse;
				bool HasEnemyLandConnection;
				int DefenseScore;
				double DistanceToUs;
			};
			std::vector<AllyCandidate> validAllies;

			HouseClass* pPrimaryEnemy = nullptr;
			if (pHouse->EnemyHouseIndex >= 0 && pHouse->EnemyHouseIndex < HouseClass::Array.Count)
				pPrimaryEnemy = HouseClass::Array[pHouse->EnemyHouseIndex];
			if (pPrimaryEnemy == nullptr)
				pPrimaryEnemy = BuildingExt::Find_Closest_Opponent(pHouse);
			...
			for (const auto pOtherHouse : HouseClass::Array)
			{
				...
				int allyBldCount = 0;
				int allyDefCount = 0;
				for (const auto pBld : pOtherHouse->Buildings)
				{
					if (pBld && pBld->IsAlive && !pBld->InLimbo)
					{
						allyBldCount++;
						if (TechTreeTypeClass::TotalBuildDefense.count(pBld->Type) > 0)
							allyDefCount++;
					}
				}

				if (allyBldCount < 2)
					continue; // Base too weak or eliminated

				bool hasEnemyLandConn = false;
				if (pPrimaryEnemy != nullptr)
					hasEnemyLandConn = GeneralUtils::AreZonesConnected(pOtherHouse->Base_Center(), pPrimaryEnemy->Base_Center(), MovementZone::Normal);

				double distToUs = pHouse->Base_Center().DistanceFrom(pOtherHouse->Base_Center());
				validAllies.push_back({pOtherHouse, hasEnemyLandConn, allyDefCount, distToUs});
			}

			// Sort allies: prefer enemy land connection first, then defense score, then closest
			std::sort(validAllies.begin(), validAllies.end(), [](const AllyCandidate& a, const AllyCandidate& b)
			{
				if (a.HasEnemyLandConnection != b.HasEnemyLandConnection)
					return a.HasEnemyLandConnection > b.HasEnemyLandConnection;

				if (a.DefenseScore != b.DefenseScore)
					return a.DefenseScore > b.DefenseScore;

				return a.DistanceToUs < b.DistanceToUs;
			});

			if (!validAllies.empty())
			{
				HouseClass* pChosenAlly = validAllies[0].pHouse;
				const bool limitByGroupAs = pPrimaryTechTree->BuildAlliedFallback_LimitByGroupAs;

				std::vector<BuildingTypeClass*> neededCandidates;
				std::set<std::string> satisfiedGroups;

				for (const auto pFallbackType : pPrimaryTechTree->BuildAlliedFallback)
				{
					if (pFallbackType == nullptr || pFallbackType->Unbuildable)
						continue;

					const char* targetGroupID = GetGroupAsID(pFallbackType);
					if (limitByGroupAs && satisfiedGroups.count(targetGroupID) > 0)
						continue;
					...
					for (const auto pBld : pHouse->Buildings)
					{
						if (pBld && pBld->IsAlive && !pBld->InLimbo)
						{
							bool match = false;
							if (limitByGroupAs)
								match = (_stricmp(GetGroupAsID(pBld->Type), targetGroupID) == 0);
							else
								match = (pBld->Type == pFallbackType);

							if (match)
							{
								for (const auto pAllyBld : pChosenAlly->Buildings)
								{
									if (pAllyBld && pAllyBld->IsAlive && !pAllyBld->InLimbo)
									{
										if (pBld->GetMapCoords().DistanceFrom(pAllyBld->GetMapCoords()) <= 25.0)
										{
											countNearAlly++;
											break;
										}
					...
					if (countNearAlly >= targetCount)
					{
						if (limitByGroupAs)
							satisfiedGroups.insert(targetGroupID);
					}
					else
					{
						neededCandidates.push_back(pFallbackType);
					}
				}

				if (!neededCandidates.empty())
				{
					// Choose randomly among the missing fallback candidates
					const int chosenIdx = ScenarioClass::Instance->Random.RandomRanged(0, static_cast<int>(neededCandidates.size()) - 1);
					BuildingTypeClass* pChosenFallbackType = neededCandidates[chosenIdx];

					if (AdvAI_Can_Build_Building(pHouse, pChosenFallbackType, true))
					{
						houseExt->TargetAlliedFallbackHouse = pChosenAlly;
						Debug::Log("AdvAI: House %d establishing/restoring Allied Fallback Outpost at House %d base -> Making AI build %s (chosen randomly from %d candidates, LimitByGroupAs: %s).\n",
								   pHouse->ArrayIndex, pChosenAlly->ArrayIndex, pChosenFallbackType->Name, static_cast<int>(neededCandidates.size()), limitByGroupAs ? "true" : "false");
						return pChosenFallbackType;
					}
					else
					{
						std::set<BuildingTypeClass*> visited;
						bool isSubPrereq = false;
						BuildingTypeClass* pPrereq = AdvAI_Find_Next_Buildable_Prerequisite(pHouse, pChosenFallbackType, pChosenFallbackType, visited, isSubPrereq);
						if (pPrereq != nullptr)
						{
							houseExt->TargetAlliedFallbackHouse = pChosenAlly;
							Debug::Log("AdvAI: House %d targeting Allied Fallback %s -> Needs prerequisite %s. Making AI build %s.\n",
									   pHouse->ArrayIndex, pChosenFallbackType->Name, pPrereq->Name, pPrereq->Name);
							return pPrereq;
						}
					}
				}
		...
		if (!skipSupport)
		{
			...
			const auto buildableSupportTypes = pPrimaryTechTree->GetBuildable(
				TechTreeTypeClass::BuildType::BuildSupport, canBuildSupportFunction);
			...
			if (!buildableSupportTypes.empty())
			{
				...
				{
					const char* groupID =
						GetGroupAsID(const_cast<BuildingTypeClass*>(pSupportType));
					...
					const SupportRadiusType supportType =
						GetSupportRadiusType(pSupportType);
					...
					const int targetBuildCount =
						GetTargetBuildCount(const_cast<BuildingTypeClass*>(pSupportType),
											-1, pPrimaryTechTree);
					const int ownedThisSpecificType = CountBuildingOfGroup(
						pHouse, const_cast<BuildingTypeClass*>(pSupportType));
					...
					// Check if we are already building this specific support structure
					// type (or an equivalent one under GroupAs)
					if (pHouse->ProducingBuildingTypeIndex != -1)
					{
						BuildingTypeClass* pProducingType =
							...
							if (pBld && pBld->IsAlive && !pBld->InLimbo)
							{
								...
								for (const auto pOtherBld : pHouse->Buildings)
								{
									if (pOtherBld && pOtherBld->IsAlive && !pOtherBld->InLimbo &&
										...
										pBld->GetMapCoords().DistanceFrom(
											pOtherBld->GetMapCoords()) <= coverageDistance)
									{
										covered = true;
										break;
									}
				...
				if (!neededSupportCandidates.empty())
				{
					BuildingTypeClass* pChosenSupport = neededSupportCandidates
						[ScenarioClass::Instance->Random.RandomRanged(
							0, static_cast<int>(neededSupportCandidates.size()) - 1)];

					Debug::Log("AdvAI: Making AI build %s for support coverage "
							   "(uncovered base buildings detected).\n",
							   pChosenSupport->Name);
		...
		if (pPrimaryTechTree->AllowNonListedBuildings)
		{
			for (const auto pBuilding : BuildingTypeClass::Array)
			{
				...
				{
					const int targetCount =
						GetTargetBuildCount(pBuilding, 1, pPrimaryTechTree);
					if (CountOwnedBuildingInstances(pHouse, pBuilding) < targetCount)
					{
						// Special case for the slave miner to count its undeployed form
						if (RulesClass::Instance->PrerequisiteProcAlternate != nullptr &&
							pBuilding->UndeploysInto ==
						...
						Debug::Log("AdvAI: Making AI build %s because it has AIBuildThis=yes "
								   "(Owned: %d, Wanted: %d).\n",
								   pBuilding->Name,
								   CountOwnedBuildingInstances(pHouse, pBuilding),
								   targetCount);
						return pBuilding;
					}
				}
		...
		// Build Service Depot if we have War Factories but not enough Service
		// Depots (1 initial + 1 per 4 refineries)
		const size_t ourRefineryCount = pPrimaryTechTree->CountSideOwnedBuildings(
			pHouse, TechTreeTypeClass::BuildType::BuildRefinery);
		const size_t optimalDepotCount = 1 + (ourRefineryCount / 4);

		if (Unsorted::CurrentFrame >
			houseExt->LastServiceDepotPlacementFailedFrame + 3000)
		{
			const BuildingTypeClass* pDepotToBuild =
				AdvAI_BuildAtLeastNOfSideAndMInTotal(
					pHouse, pPrimaryTechTree,
					TechTreeTypeClass::BuildType::BuildServiceDepot, 1,
					optimalDepotCount);
			if (pDepotToBuild != nullptr)
			{
				Debug::Log(
					"AdvAI: Making AI build %s (Refinery count: %d, Max depots: %d).\n",
					pDepotToBuild->Name, static_cast<int>(ourRefineryCount),
					static_cast<int>(optimalDepotCount));
	...
	///	Make the AI build other tech if it isn't too busy
	{
		const BuildingTypeClass* pPowerPlantToBuild =
			AdvAI_BuildAtLeastNOfSideAndMInTotal(
				pHouse, pSecondaryTechTree,
				TechTreeTypeClass::BuildType::BuildPower, 1, 0);
		if (pPowerPlantToBuild != nullptr)
		{
			Debug::Log(
				"AdvAI: Making AI build %s because it is currently developing its "
				"tech tree for side %d and does not have a basic power plant.\n",
		...
		const BuildingTypeClass* pBarracksToBuild =
			AdvAI_BuildAtLeastNOfSideAndMInTotal(
				pHouse, pSecondaryTechTree,
				TechTreeTypeClass::BuildType::BuildBarracks, 1, 0);
		if (pBarracksToBuild != nullptr)
		{
			Debug::Log("AdvAI: Making AI build %s because it is currently developing "
					   "its tech tree for side %d and does not have a barracks.\n",
		...
		const BuildingTypeClass* pRefineryToBuild =
			AdvAI_BuildAtLeastNOfSideAndMInTotal(
				pHouse, pSecondaryTechTree,
				TechTreeTypeClass::BuildType::BuildRefinery, 1, 0);
		if (pRefineryToBuild != nullptr)
		{
			Debug::Log("AdvAI: Making AI build %s because it is currently developing "
					   "its tech tree for side %d and does not have a refinery.\n",
					   pRefineryToBuild->Name, pSecondaryTechTree->SideIndex.Get());
			return pRefineryToBuild;
		}

		const BuildingTypeClass* pWeaponsToBuild =
			AdvAI_BuildAtLeastNOfSideAndMInTotal(
				pHouse, pSecondaryTechTree,
				TechTreeTypeClass::BuildType::BuildWeapons, 1, 0);
		if (pWeaponsToBuild != nullptr)
		{
			Debug::Log(
				"AdvAI: Making AI build %s because it is currently developing its "
				"tech tree for side %d and does not have a weapons factory.\n",
		...
		const BuildingTypeClass* pNavalYardToBuild =
			AdvAI_BuildAtLeastNOfSideAndMInTotal(
				pHouse, pSecondaryTechTree,
				TechTreeTypeClass::BuildType::BuildNavalYard, 1, 0);
		if (pNavalYardToBuild != nullptr)
		{
			Debug::Log("AdvAI: Making AI build %s because it is currently developing "
					   "its tech tree for side %d and does not have a naval yard.\n",
		...
		const BuildingTypeClass* pRadarToBuild =
			AdvAI_BuildAtLeastNOfSideAndMInTotal(
				pHouse, pSecondaryTechTree,
				TechTreeTypeClass::BuildType::BuildRadar, 1, 0);
		if (pRadarToBuild != nullptr)
		{
			Debug::Log("AdvAI: Making AI build %s because it is currently developing "
					   "its tech tree for side %d and does not have a radar.\n",
		...
		const BuildingTypeClass* pHelipadToBuild =
			AdvAI_BuildAtLeastNOfSideAndMInTotal(
				pHouse, pSecondaryTechTree,
				TechTreeTypeClass::BuildType::BuildHelipad, 1, 0);
		if (pHelipadToBuild != nullptr)
		{
			Debug::Log("AdvAI: Making AI build %s because it is currently developing "
					   "its tech tree for side %d and does not have a helipad.\n",
		...
		const BuildingTypeClass* pTechCenterToBuild =
			AdvAI_BuildAtLeastNOfSideAndMInTotal(
				pHouse, pSecondaryTechTree, TechTreeTypeClass::BuildType::BuildTech,
				1, 0);
		if (pTechCenterToBuild != nullptr)
		{
			Debug::Log("AdvAI: Making AI build %s because it is currently developing "
					   "its tech tree for side %d and does not have a tech center.\n",
		...
		for (const auto buildOtherPair : pSecondaryTechTree->BuildOtherCountMap)
		{
			...
			const int targetCount = GetTargetBuildCount(
				buildOtherPair.first, buildOtherPair.second, pSecondaryTechTree);
			if (CountOwnedBuildingInstances(
					pHouse, buildOtherPair.first) < targetCount)
			{
				Debug::Log("AdvAI: Making AI build %s because it does not have enough "
	...
	// Are there other AIBuildThis=yes buildings that we haven't built yet?
	if (!pPrimaryTechTree || pPrimaryTechTree->AllowNonListedBuildings)
	{
		for (const auto pBuilding : BuildingTypeClass::Array)
		{
			// Exclude defenses here, no need to build defenses just to have them
			...
			if (AdvAI_Can_Build_Building(pHouse, pBuilding, true))
			{
				const int targetCount = GetTargetBuildCount(pBuilding, 1, pPrimaryTechTree);
				if (CountOwnedBuildingInstances(pHouse, pBuilding) <
					targetCount)
				{
					// Special case for the slave miner to count its undeployed form
					if (RulesClass::Instance->PrerequisiteProcAlternate != nullptr &&
		...
		houseExt->NextExpansionPointLocation.Y != 0)
	{
		...
		for (const auto pType : TechTreeTypeClass::TotalBuildPower)
		{
			...
			{
				if (pType->Cost < lowestCost ||
					(pType->Cost == lowestCost && pType->Adjacent > maxAdjacent))
				{
				...
				else if (pType->Cost == lowestCost &&
						 pType->Adjacent == maxAdjacent)
				{
					bestPowerCandidates.push_back(pType);
				}
			}
...
const BuildingTypeClass* HouseExt::AdvAI_BuildAtLeastNOfSideAndMInTotal(
	HouseClass* pHouse, TechTreeTypeClass* techTree,
	TechTreeTypeClass::BuildType buildType, int sideBuildingsWanted,
	int totalBuildingsWanted, int extraCount)
{
	auto canBuildFunction = [pHouse](auto&& PH1)
	{
		return AdvAI_Can_Build_Building(pHouse, std::forward<decltype(PH1)>(PH1),
	...
	const BuildingTypeClass* pOurBuilding =
		techTree->GetRandomBuildable(buildType, canBuildFunction);
	const size_t ourBuildingCount =
		techTree->CountSideOwnedBuildings(pHouse, buildType) + extraCount;
	const size_t totalBuildingCount =
		TechTreeTypeClass::CountTotalOwnedBuildings(pHouse, buildType) +
		extraCount;
...
const BuildingTypeClass*
HouseExt::AdvAI_Get_Building_To_Build(HouseClass* pHouse)
{
	const auto houseExt = ExtMap.Find(pHouse);
	if (houseExt != nullptr && !houseExt->UnclaimedTiberiumZones.empty() &&
		!AdvAI_IsMobileRefineryHouse(pHouse))
	{
		const auto pTechTree = TechTreeTypeClass::GetAnySuitable(pHouse);
		if (pTechTree != nullptr && !pTechTree->BuildRefinery.empty())
		{
			...
			if (pRefinery != nullptr &&
				AdvAI_Can_Build_Building(pHouse, pRefinery, true, true))
			{
				for (auto it = houseExt->UnclaimedTiberiumZones.begin();
					 it != houseExt->UnclaimedTiberiumZones.end(); ++it)
				{
					CellStruct target = *it;
					if (AdvAI_Is_Failed_Expansion_Point(pHouse, target))
					{
						continue; // Skip targets currently under cooldown (failed
								  // placements)
					}
					...
					bool isFailedOrBlocked =
						AdvAI_Has_Failed_Placement_Three_Times(pHouse, target);

					bool hasRefinery = false;
					for (const auto pBld : BuildingClass::Array)
					{
						if (pBld && pBld->IsAlive && !pBld->InLimbo &&
							pBld->Owner == pHouse && pBld->Type->ResourceDestination)
						{
					...
					CellStruct bestCell =
						BuildingExt::Get_Best_Expansion_Placement_Position_Helper(
							pHouse, pRefinery, nullptr);
					double bestDistToTarget = (bestCell.X > 0 && bestCell.Y > 0)
												  ? bestCell.DistanceFrom(target)
												  : 9999.0;

					if (bestDistToTarget <= idealMaxDist || isFailedOrBlocked)
					{
						// Excellent: placement is within ideal distance (or crawling failed
						// so we accept fallback placement)!
						Debug::Log("AdvAI: Intercepting build loop to construct refinery "
								   "%s for unclaimed tiberium zone at (%d,%d) (Placement "
								   "dist: %.1f cells <= %.1f).\n",
								   pRefinery->Name, target.X, target.Y, bestDistToTarget,
								   coverageRange);
						return pRefinery;
					}
					else
					{
						// Placement is farther than 11.0 cells (e.g. 16+ cells away). Crawl
						// closer first!
							...
							houseExt->NextExpansionPointLocation.Y == 0)
						{
							...
							Debug::Log("AdvAI: Unclaimed tiberium zone at (%d,%d) is too far "
									   "for direct refinery (best dist: %.1f > %.1f). "
									   "Crawling closer first.\n",
	...
	const BuildingTypeClass* buildChoice =
		AdvAI_Evaluate_Get_Best_Building(pHouse);

	if (buildChoice == nullptr)
	{
		// Check if we still need power plants because real power surplus is low or buildings are unpowered
		const int realDrain = GetRealPowerDrain(pHouse);
		const int requiredSurplus = pHouse->PowerSurplus > 0
										? pHouse->PowerSurplus
										: RulesClass::Instance->PowerSurplus;
		if (pHouse->PowerOutput - realDrain < requiredSurplus)
		{
			const auto pTechTree = TechTreeTypeClass::GetAnySuitable(pHouse);
			if (pTechTree != nullptr)
			{
				...
				const BuildingTypeClass* pAdvPower = pTechTree->GetRandomBuildable(
					TechTreeTypeClass::BuildType::BuildAdvancedPower, canBuildFunction);
				if (pAdvPower != nullptr)
				{
					Debug::Log("AdvAI: House %d building %s to restore full power surplus for deactivated structures.\n",
							   pHouse->ArrayIndex, pAdvPower->Name);
					return pAdvPower;
				}

				const BuildingTypeClass* pPower = pTechTree->GetRandomBuildable(
					TechTreeTypeClass::BuildType::BuildPower, canBuildFunction);
				if (pPower != nullptr)
				{
					Debug::Log("AdvAI: House %d building %s to restore full power surplus for deactivated structures.\n",
							   pHouse->ArrayIndex, pPower->Name);
					return pPower;
				}
			}
		}
	...
	if (buildChoice->PowerDrain > 0 && !buildChoice->ResourceDestination &&
		!buildChoice->ConstructionYard)
	{
		const int expectedSurplus =
			(pHouse->PowerOutput - GetRealPowerDrain(pHouse)) -
			buildChoice->PowerDrain;
		const int requiredSurplus = pHouse->PowerSurplus > 0
										? pHouse->PowerSurplus
										: RulesClass::Instance->PowerSurplus;
		...
		if (expectedSurplus < requiredSurplus)
		{
			...
			if (pTechTree != nullptr)
			{
				...
				if (pAdvPower != nullptr)
				{
					Debug::Log("AdvAI: Intercepted building %s to build %s first due to "
							   "insufficient power surplus.\n",
...
static bool IsLocatedInAlliedBase(BuildingClass* pBuilding,
								  HouseClass* pHouse)
{
...
void HouseExt::AdvAI_Economy_Upkeep(HouseClass* pHouse)
{
	...
	for (const auto pHarvester : RulesClass::Instance->HarvesterUnit)
		harvesterCount +=
			pHouse->ActiveUnitTypes.GetItemCount(pHarvester->ArrayIndex);
	...
	Debug::Log(
		"AdvAI: Looking for a refinery to sell because we have %d excess.\n",
		toSellCount);

	// Sell the refinery that is closest to our primary enemy.
	// If we have extra refineries, we have lost harvesters, and harvesters are
	// most likely lost near the expansion that is closest to our primary enemy.
	...
	for (const auto pBuilding : BuildingClass::Array)
	{
		...
		if (pBuilding->CurrentMission == Mission::Construction ||
			pBuilding->QueuedMission == Mission::Construction)
		{
			// If a refinery is in process of being constructed, it hasn't got the
			// spawn its FreeUnit harvester yet.
		...
		const double distance = centerPoint.DistanceFrom(
			GeneralUtils::CellFromCoordinates(pBuilding->GetCenterCoords()));
...
/**
 *  Sells extra construction yards of the specific house until there is one one
 * left.
 *
 *  Author: Rampastring
 */
void HouseExt::AdvAI_Sell_Extra_ConYards(HouseClass* pHouse)
{
	...
	Debug::Log("AdvAI: AI %d has too many Construction Yards (%d). Selling off "
			   "%d of them. Frame: %d\n",
	...
	for (int i = pHouse->ConYards.Count - 1; i > 0; i--)
	{
		...
		if (building->CurrentMission == Mission::Selling ||
			building->QueuedMission == Mission::Selling)
		{
			soldCount++;
...
bool HouseExt::IsAdvancedAIActive(HouseClass* pHouse)
{
 ...
 *  Author: Rampastring
 */
void HouseExt::AdvAI_Building(HouseClass* pHouse)
{
	...
	// Sync the previous ShouldBuildRefinery state back to
 ...
 *  Author: Rampastring
 */
void HouseExt::AdvAI_ExpertAI(HouseClass* pHouse)
{
	...
	// If we have more than 1 ConYard without Rules allowing it, sell some of them
	...
	int paranoiaDuration = TICKS_PER_MINUTE;
	if (pHouse->LATime == 0 ||
		pHouse->LATime + paranoiaDuration + 1800 <= Unsorted::CurrentFrame)
	{
	...
	if (Unsorted::CurrentFrame >
		houseExt->LastObsoleteRefineryCheckFrame + 4500)
	{
	...
	if (Unsorted::CurrentFrame >
		houseExt->LastSleepingHarvesterCheckFrame + 1000)
	{
	...
	if (Unsorted::CurrentFrame >
		houseExt->LastUnclaimedTiberiumCheckFrame + 450)
	{
	...
	if (Unsorted::CurrentFrame > 5000 && pHouse->ConYards.Count == 0 &&
		pHouse->NumWarFactories == 0 && !houseExt->HasPerformedVehicleCharge)
	{
		...
		for (const auto pUnit : UnitClass::Array)
		{
			if (pUnit->Owner == pHouse &&
				(pUnit->Type->DeploysInto == nullptr ||
				 !pUnit->Type->DeploysInto->ConstructionYard) &&
				!pUnit->Type->Harvester && !pUnit->Type->Weeder)
			{
	...
	// If we are under threat of getting rushed early and our ConYard is producing
	// something non-defensive and non-power-granting, abandon it.
	const int enemyAircraftCount = AdvAI_Calculate_Enemy_Aircraft_Value(pHouse);
	const bool isUnderThreat =
		AdvAI_Is_Under_Start_Rush_Threat(pHouse, enemyAircraftCount);

	if (isUnderThreat)
	{
		FactoryClass* buildingFactory = pHouse->GetPrimaryFactory(
...
void HouseExt::AdvAI_Update_Primary_Factories(HouseClass* pHouse)
{
	...
	auto updatePrimary = [pHouse, targetCoords](AbstractType type, bool isNaval)
	{
		...
		for (const auto pBuilding : pHouse->Buildings)
		{
			if (pBuilding && pBuilding->IsAlive && pBuilding->Type->Factory == type &&
				pBuilding->Type->Naval == isNaval && pBuilding->IsPrimaryFactory)
			{
		...
		// Fallback: If no land factory can reach the target (e.g. they are all on
		// the main island and the target is on another island), fall back to
		// distance check to make sure at least some factory remains primary.
...
void HouseExt::AdvAI_Recycle_Furthest_Factory(HouseClass* pHouse,
											  AbstractType factoryType,
											  bool isNaval, size_t optimalCount,
											  CellStruct targetCell)
{
	...
	// Check if we have a foothold near the target (any building of ours
	// within 22.0 cells of targetCell)
	...
	if (!hasFoothold)
		return; // Cannot build close to the target anyway, wait for crawler to
				// advance
	...
	for (const auto pFactory : ourFactories)
	{
		...
		const double conyardDist =
			pFactory->GetMapCoords().DistanceFrom(conyardCell);
		if (conyardDist > 20.0)
		{
	...
	if (nearestDist > 22.0 && pFurthestFactory != nullptr &&
		furthestDist > 35.0)
	{
		Debug::Log("AdvAI Recycling: House %d selling furthest %s %s factory %s at "
				   "(%d,%d) (dist %.1f from target) to rebuild closer (nearest is "
				   "%.1f).\n",
				   pHouse->ArrayIndex, (isNaval ? "Naval" : "Land"),
				   (factoryType == AbstractType::InfantryType ? "Infantry"
															  : "Unit/Vehicle"),
				   pFurthestFactory->Type->ID, pFurthestFactory->GetMapCoords().X,
				   pFurthestFactory->GetMapCoords().Y, furthestDist, nearestDist);
...
void HouseExt::AdvAI_Recycle_Obsolete_Refineries(HouseClass* pHouse)
{
	...
	for (const auto pBld : pHouse->Buildings)
	{
		...
		// Scan a radius of 14 cells around the refinery to check for tiberium trees
		// or tiberium overlays.
		const CellStruct refCoords =
			GeneralUtils::CellFromCoordinates(pBld->GetCenterCoords());
			...
			for (int dx = -27; dx <= 27; ++dx)
			{
				...
				if (cell)
				{
					...
					if (within22)
					{
						...
						if (pTerrain != nullptr && pTerrain->IsAlive &&
							pTerrain->Type->SpawnsTiberium)
						{
							hasResources = true;
							break;
		...
		if (!hasResources)
		{
			Debug::Log("AdvAI: Refinery %s at (%d,%d) is obsolete (no resources "
					   "within 27 cells, no trees within 22 cells). Selling it.\n",
...
void HouseExt::AdvAI_Add_Failed_Expansion_Point(HouseClass* pHouse,
												CellStruct coords)
{
	const auto houseExt = ExtMap.Find(pHouse);
		 ...
		 i++)
	{
		...
		if (blocked.Coords.X > 0 && blocked.Coords.Y > 0)
		{
			if (coords.DistanceFromSquared(blocked.Coords) < 25.0) // 5-cell radius
			{
				...
				// Cap multiplier at 240 to prevent extreme values (240 * 3 mins = 12
				// hours)
				int multiplier = std::min(blocked.FailureCount, 240);

				blocked.ExpiryFrame = Unsorted::CurrentFrame + (2700 * multiplier);

				Debug::Log("AdvAI: House %d updated failed expansion point (%d,%d). "
						   "Failure count: %d, multiplier: %dx, cooldown: %d mins.\n",
						   pHouse->ArrayIndex, coords.X, coords.Y, blocked.FailureCount,
	...
	if (!found)
	{
			 ...
			 i++)
		{
			...
			if (Unsorted::CurrentFrame >= b.ExpiryFrame &&
				b.ExpiryFrame < oldestExpiry)
			{
		...
		{
			auto& blocked =
				houseExt->PermanentlyBlockedExpansionPointLocations[bestIndex];
			blocked.Coords = coords;
			blocked.FailureCount = 1;
			blocked.ExpiryFrame =
				Unsorted::CurrentFrame + 2700; // 3 minute cooldown at 15 FPS
			Debug::Log("AdvAI: House %d blacklisted failed expansion point (%d,%d) "
					   "for 3 minutes.\n",
											   ...
											   CellStruct coords)
{
	...
	{
		const auto& blocked =
			houseExt->PermanentlyBlockedExpansionPointLocations[i];
		if (blocked.Coords.X > 0 && blocked.Coords.Y > 0)
		{
			// Entry is only active if the frame hasn't reached its individual expiry
			// frame
...
bool HouseExt::AdvAI_Has_Failed_Placement_Three_Times(HouseClass* pHouse,
													  CellStruct coords)
{
	const auto houseExt = ExtMap.Find(pHouse);
	for (size_t i = 0;
...
void HouseExt::AdvAI_Update_Unclaimed_Tiberium_Zones(HouseClass* pHouse)
{
	...
	if (!zones.empty())
		Debug::Log("AdvAI: Update_Unclaimed_Tiberium_Zones for House %d. Current "
				   "tracked zones count: %d\n",
				   pHouse->ArrayIndex, static_cast<int>(zones.size()));

	zones.erase(
		std::remove_if(
			zones.begin(), zones.end(),
			[pHouse](const CellStruct& coords)
			{
				...
				if (AdvAI_Has_Failed_Placement_Three_Times(pHouse, coords))
				{
					if (failedIdx >= 0)
						Debug::Log(
							"AdvAI: Sector #%d at (%d,%d) has failed refinery "
							"placement 3 times. Removing permanently from tracking.\n",
							failedIdx, coords.X, coords.Y);
					else
						Debug::Log(
							"AdvAI: Tree Node at (%d,%d) has failed refinery placement "
							"3 times. Removing permanently from tracking.\n",
							coords.X, coords.Y);
					return true; // remove
				}
				...
				for (int dy = -4; dy <= 4; ++dy)
				{
					...
					{
						CellStruct cellCoords(static_cast<short>(coords.X + dx),
											  static_cast<short>(coords.Y + dy));
						if (!MapClass::Instance.CoordinatesLegal(cellCoords))
							continue;

						const CellClass* cell =
							MapClass::Instance.GetCellAt(cellCoords);
						if (cell)
						{
							if (cell->OverlayTypeIndex != -1 &&
								OverlayClass::GetTiberiumType(cell->OverlayTypeIndex) >=
									0)
							{
				...
				if (!stillHasTiberium)
				{
					if (sIdx >= 0)
						Debug::Log(
							"AdvAI: Unclaimed tiberium zone Sector #%d at (%d,%d) has "
							"been mined out or cleared. Removing from tracking.\n",
							sIdx, coords.X, coords.Y);
					else
						Debug::Log(
							"AdvAI: Unclaimed tiberium Tree Node at (%d,%d) has been "
							"mined out or cleared. Removing from tracking.\n",
							coords.X, coords.Y);
					return true; // remove
				}
				...
				for (const auto pTerrain : TerrainClass::Array)
				{
					if (pTerrain->IsAlive && !pTerrain->InLimbo &&
						pTerrain->Type->SpawnsTiberium &&
						pTerrain->GetMapCoords() == coords)
					{
						isTree = true;
						break;
					}
				...
				{
					if (sIdx >= 0)
						Debug::Log("AdvAI: Refinery now exists near Sector #%d at "
								   "(%d,%d). Removing from tracking.\n",
								   sIdx, coords.X, coords.Y);
					else
						Debug::Log("AdvAI: Refinery now exists near Tree Node at "
								   "(%d,%d). Removing from tracking.\n",
								   coords.X, coords.Y);
					return true; // remove
				}
	...
	if (!candidates.empty())
		Debug::Log("AdvAI UnclaimedZonesScan: House %d: Scanned %d candidates "
				   "(ground sectors + trees).\n",
	...
	for (const auto& candidate : candidates)
	{
		...
		if (pClosestBld != nullptr &&
			!GeneralUtils::AreZonesConnected(pClosestBld->GetMapCoords(),
											 targetCoords, MovementZone::Normal))
		{
			continue; // Skip Tiberium zones separated from base by cliffs, water, or
					  // impassable barriers
		}
		...
		// Check if already in UnclaimedTiberiumZones or within 15 cells of an
		// existing registered zone
		...
		if (sIdx >= 0)
			Debug::Log("AdvAI: Registered new unclaimed Sector #%d at (%d,%d) near "
					   "building %s (distance: %.1f < %.1f). Vector size: %d.\n",
					   sIdx, targetCoords.X, targetCoords.Y,
					   pClosestBld ? pClosestBld->Type->ID : "???", minBldDist,
					   maxBaseDistance, static_cast<int>(zones.size()));
		else
			Debug::Log("AdvAI: Registered new unclaimed Tree Node at (%d,%d) near "
					   "building %s (distance: %.1f < %.1f). Vector size: %d.\n",
...
CellStruct HouseExt::ExtData::GetCrawlingWaypoint(CellStruct targetCell)
{
	...
	CellStruct currentStartCoords =
		pStartBld ? pStartBld->GetMapCoords() : CellStruct(0, 0);
	...
	auto& cachedPath =
		isCombatTarget ? this->CachedCombatPath : this->CachedResourcePath;
	auto& cachedPathTarget = isCombatTarget ? this->CachedCombatPathTarget
											: this->CachedResourcePathTarget;
		...
		cachedPath.empty())
	{
		...
		if (cachedPathTarget == targetCell && !cachedPath.empty() &&
			pStartBld != nullptr)
		{
			// Check if currentStartCoords is close to any node on the existing cached
			// path
			...
			{
				const double dSq =
					currentStartCoords.DistanceFromSquared(cachedPath[k]);
		...
		if (!reusedExistingPath)
		{
			...
			{
				cachedPath = GeneralUtils::GetAStarPath(currentStartCoords, targetCell,
														MovementZone::Normal);
				cachedPathTarget = targetCell;
				cachedPathStart = currentStartCoords;

				if (!cachedPath.empty())
					Debug::Log("AdvAI: Recalculated A* crawl path (%s) from (%d,%d) to "
							   "(%d,%d). Path size: %d cells.\n",
							   isCombatTarget ? "Combat" : "Resource",
							   currentStartCoords.X, currentStartCoords.Y, targetCell.X,
							   targetCell.Y, static_cast<int>(cachedPath.size()));
			}
			else
			{
				cachedPath.clear();
